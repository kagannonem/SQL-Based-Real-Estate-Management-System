...
def create_property(p: PropertyCreateInput):
...
def update_property_status(property_id: int, body: StatusUpdateInput):
...
def create_client(c: ClientCreateInput):
...
def get_listing_viewing_counts():
...
def create_listing(l: ListingCreateInput):
...
def create_viewing(v: ViewingCreateInput):
...
def delete_viewing(viewing_id: int):
...
def create_transaction(t: TransactionCreateInput):
...
def create_office(o: OfficeCreateInput):
